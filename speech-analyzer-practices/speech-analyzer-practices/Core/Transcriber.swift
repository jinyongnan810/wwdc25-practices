...
//
//  Created by Yuunan kin on 2025/07/12.
//
...
final class Transcriber: Sendable {
    ...
    init(story: Binding<Story>) {
        self.story = story
        print("building transcriber. story: \(story)")
    }

    func setupTranscriber() async throws {
        print("setting up transcriber. locale: \(Locale.current)")
        ...
        guard let transcriber else {
            print("failed to setup transcriber")
        ...
        print("analyer format: \(String(describing: analyzerFormat))")

        guard let inputSequence else {
            print("no inputSequence")
            return
        }
        ...
        recognizerTask = Task {
            do {
                for try await case let result in transcriber.results {
                    ...
                    print("transcriber text: \(text), is final: \(result.isFinal)")
                    if result.isFinal {
